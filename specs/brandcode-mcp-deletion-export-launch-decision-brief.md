...
**Status:** Deletion/export operating posture accepted; launch language still
...
accepted the recommended deletion/export operating posture: narrow requester
classes, role-based or recorded-contact verification, hosted MCP service scope,
clear exclusions, curated support-packet exports, no public SLA, Jason as
pre-release escalation owner, and legal review before public launch language.

This brief records the accepted operating posture. It does not approve public
release, public directory copy, self-serve deletion/export, public SLA, or
legal terms.
...
- Authority proof is brand instance admin access, or verified email matching
  ...
  Brandcode instance.
- For non-internal brands, authority proof is brand instance admin role, an
  approved client contact on record, or written approval from an admin/contract
  owner through the support ledger. Email-domain match alone is a signal, not
  sufficient proof.

## Accepted Operating Decisions

| Area | Accepted posture | Remaining launch blocker |
| --- | --- | --- |
| Requester authorization | Brand owner/admin for the brand instance; named legal/contract contact if recorded for that client; Jason Lankow as Brandcode Studio Ops. Agency/vendor requesters require explicit recorded authorization. | Legal-approved public wording. |
| Authorization verification | Brand instance admin role; approved client contact on record; or written approval from an admin/contract owner through the support ledger. For internal Column Five Brandcode, verified `columnfivemedia.com` or `columnfive.com` email may be used. Email-domain match alone is not sufficient for external clients. | Legal-approved public wording and evidence-retention wording. |
| Data scope | Hosted MCP service data: `brand_feedback`, scoped `brand_history` summaries/receipts, limited-client support/intake records, non-secret key metadata, and MCP-visible package/runtime references. Canonical Brandcode Studio records, official Full Brand Runtime packages, billing/legal records, and private provider assets stay on separate Brandcode/UCS/admin paths. | Legal-approved public wording. |
| Exclusions | Security/abuse logs, rate-limit counters, audit receipts/tombstones, billing/legal records, legal holds, backups until normal expiry, secrets, raw private/provider URLs, private custody paths, and third-party/provider data outside Brandcode custody are excluded from ordinary deletion/export. Deletion may be deletion, redaction, tombstone, or annotation depending on system and audit requirements. | Legal-approved retention, backup, and excluded-record language. |
| Export package format | Curated support packet: Markdown index plus JSON summaries/receipts, support/intake records, non-secret key metadata, and package-safe refs. No raw nested UCS blobs by default, no secrets, no private/provider URLs, and no public delivery links. | Legal-approved delivery-channel wording. |
| Response windows | No public SLA or customer-facing completion timeline during pre-release. Manual review only; future public posture may start with acknowledgement language, not completion commitments. | Legal/ops-approved acknowledgement or response language if any. |
| Escalation path | Jason Lankow / Brandcode Studio Ops for pre-release. Before public launch, name a backup owner and legal/privacy reviewer. | Backup owner and legal/privacy reviewer. |
| Legal/subprocessor language | Draft language below is accepted as useful review input only. | Final terms, privacy, DPA, and subprocessor language must be approved before public launch copy. |

## Recommended Operating Shape

For v0.1 limited-client operations, use the conservative approved-client
posture above:

1. Requesters must be a brand owner/admin for the brand instance, or Jason
   Lankow as Brandcode Studio Ops.
2. Authorization must be verified through brand instance admin status, approved
   client contact on record, written approval from an admin/contract owner
   through the support ledger, or, for the internal Column Five Brandcode
   instance, verified email matching `columnfivemedia.com` or
   `columnfive.com`. Domain match alone is not sufficient for external
   clients.
...
This accepted posture is not public launch copy.
...
| Legal/subprocessor requirement | Draft language accepted as useful review input; final terms/privacy/DPA/subprocessor language not approved |
